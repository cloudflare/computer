...
//
//   POST /prompt ──► RecipeAgent (AIChatAgent + Workspace)
//                      │ webfetch openstove.org           (host)
...
// the host once the exec finishes.
//
...
const SYSTEM = [
  ...
  "",
  `1. Find the recipe. \`webfetch\` ${SITEMAP} lists every recipe page.`,
  ...
  "   text alone.",
  `3. Convert it: \`exec\` \`pandoc ${MARKDOWN} -o ${PDF} --pdf-engine=typst\`.`,
    ...
    const result = streamText({
      ...
      tools: {
        webfetch: tool({
          ...
          execute: async ({ url }) => {
            // An allowlist of one host, checked on the parsed URL rather
            // than the string, so a prompt can't talk the agent into
            // fetching somewhere else. Redirects aren't followed: one off
            // the allowed host would quietly undo the check.
        ...
        exec: tool({
          description:
            "Run a shell command in the container. Real Linux: pandoc and typst " +
            "are on the path and the workspace is mounted at /workspace.",
