 ...
 *
 * The shell is real: `just-bash`, the same implementation the
 * `worker-shell` backend runs inside its Dynamic Worker, driven
 * against just-bash's own in-memory filesystem wrapped in a recorder.
 * Whether `find -delete` reaches for a delete is a fact about
 * just-bash and holds wherever its files happen to live, so the
 * storage underneath does not need to be the real Durable Object for
 * the answer to be right.
 *
 * The other two backends are not exercised, and neither has a claim
 * here to exercise. `container-shell` runs GNU coreutils rather than
 * just-bash, so the same command can behave differently there;
 * `worker-javascript` evaluates a module, which this matcher does not
 * read at all. The default policy gates both outright, which is why
 * the difference does not bite, and is a reason to keep gating them.
 *
 * What this file does not test, and what nothing in this example
 * needs it to test, is whether a write the policy misjudged actually
 * lands. That is the workspace's job: a command that was not approved
 * runs without write access, and the filesystem refuses it whatever
 * this matcher concluded. This file is about how many questions get
 * asked, not about what happens when the answer is wrong.
 */
...
describe("the detector itself", () => {
  ...
  it("would catch a policy that waved a write through", async () => {
    ...
    const policy = { rules: { "worker-shell": "never" as const } };
    const command = "rm -rf /workspace/sub";
    expect(decideApproval({ command, backend: "worker-shell" }, policy).needsApproval).toBe(false);
...
describe("every command the policy allows unattended", () => {
  it("writes nothing", async () => {
    ...
    for (const command of corpus()) {
      if (decideApproval({ command, backend: "worker-shell" }).needsApproval) continue;
      allowed += 1;
