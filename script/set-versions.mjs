...
//
// Both @cloudflare/computer and @cloudflare/computer-computerd-linux-x64
...
const PACKAGES = [
  ...
  "packages/computer-computerd-linux-x64/package.json",
  // computerd itself stays private, but its package.json version is what
  // the build-docker.mjs script reads to tag the published image.
  // Keeping it in lockstep means the docker tag matches the npm
  // tag.
  "packages/computerd/package.json",
];

// Example Dockerfiles pin a specific
// ghcr.io/cloudflare/computer-computerd-linux-x64:<version> in their
...
const COMPUTERD_IMAGE_TAG_RE = /(ghcr\.io\/cloudflare\/computer-computerd-linux-x64:)[^\s]+/g;
...
for (const dockerfile of DOCKERFILES) {
  ...
  if (!COMPUTERD_IMAGE_TAG_RE.test(before)) {
    console.error(
      `${dockerfile}: no computerd-linux-x64 image tag matched ${COMPUTERD_IMAGE_TAG_RE}`,
    );
    process.exit(2);
  }
  // RegExp with /g keeps lastIndex between calls; reset before replace().
  COMPUTERD_IMAGE_TAG_RE.lastIndex = 0;
  const after = before.replace(COMPUTERD_IMAGE_TAG_RE, `$1${version}`);
  if (after !== before) await writeFile(dockerfile, after);
  console.log(`${dockerfile}: computerd image tag → ${version}`);
}
